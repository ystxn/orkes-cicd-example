...
process_file() {
    ...
    echo "Deploying $resource_type $file_path"
    ...
    echo "Calling POST $TARGET_CLUSTER/$endpoint"
    local response_code=$(curl -sS -w "%{http_code}" -X POST -H "Content-Type: application/json" -H "X-Authorization: $TOKEN" -d "$json_content" "$TARGET_CLUSTER/$endpoint" 2>/dev/null || echo 1)
